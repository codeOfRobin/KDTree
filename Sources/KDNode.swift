...
public class KDNode<Element: LocationValue, T: BoundingValues> {
    ...
    init?(values: [Element], parent: KDNode?) {
        ...
        self.parent = parent
        location = centroidLocationFromValues(values: values)

        setChildren(values: values)
    }

    func setChildren(values: [Element]) {
        let split = splitValues(values: values)
    ...
    func splitValues(values: [Element]) -> (left: [Element], right: [Element]) {
        ...
        for value in values {
            if shouldLocateLeft(value: value) {
                left.append(value)
            } else {
                right.append(value)
            }
    ...
    func shouldLocateLeft(value: Element) -> Bool {
        ...
        if let compare = compare, let average = average, compare < average {
            return true
        }
        return false
    }
...
extension KDNode {
    ...
    func insert(value: Element) {
        ...
        if isLeaf {
            setChildren(values: [location, value])
            return
        }
    ...
    func remove(value: Element) {
        ...
        if isLeaf && value == location {
            parent?.removeChildLeafNode(node: self)
            return
        }
...
extension KDNode {
    ...
    func isAncestor(node: KDNode) -> Bool {
        ...
        return left == node || right == node || left?.isAncestor(node: node) == true || right?.isAncestor(node: node) == true
    }

    func isDescendant(node: KDNode) -> Bool {
        return node.isAncestor(node: self)
    }
}
...
extension KDNode {
    ...
    func forEachLeaf(each: (_ : KDNode) -> Void) {

        left?.forEachLeaf(each: each)
        if isLeaf {
            each(self)
        }
        right?.forEachLeaf(each: each)
    }
}
